      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-[var(--line)] px-6 py-5 bg-[var(--surface)]">
          <div className="flex items-center gap-3">
            ...
            <Typography className="text-[1.15rem] tracking-tight" variant="title-md">Seu carrinho</Typography>
            {state.items.length > 0 && (
              <Badge className="bg-[var(--brand-orange)] px-2.5 py-0.5 text-xs font-black text-white shadow-sm" tone="orange">
                {state.items.reduce((s, i) => s + i.quantity, 0)}
              </Badge>
            )}
          </div>
          <IconButton className="p-2.5 hover:bg-[var(--brand-orange)]/10 hover:text-[var(--brand-orange-dark)]" label="Fechar carrinho" onClick={closeCart}>
            <svg
              aria-hidden="true"
              className="h-5 w-5"
              fill="none"
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 scrollbar-thin scrollbar-thumb-[var(--brand-orange)]/20">
          {state.items.length === 0 ? (
            <EmptyState className="flex flex-col items-center justify-center gap-5 border-none bg-transparent py-20">
              ...
              <div className="space-y-1">
                <Typography className="text-[1.2rem]" variant="title-md">Carrinho vazio</Typography>
                <Typography className="text-[0.95rem]" tone="muted" variant="body-sm">Que tal adicionar um burger artesanal?</Typography>
              </div>
              <Button className="mt-2 px-8 py-3.5 text-[0.95rem] shadow-[0_4px_14px_rgba(242,122,34,0.3)] hover:-translate-y-0.5 hover:shadow-[0_6px_20px_rgba(242,122,34,0.4)] active:translate-y-0" onClick={closeCart}>
                Ver cardápio
              </Button>
            </EmptyState>
          ) : (
                ...
                >
                  ...
                  <div className="flex min-w-0 flex-1 flex-col gap-1">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <Typography className="text-[0.9rem] leading-snug" variant="body-sm">{item.name}</Typography>
                      <button
                    ...
                    {item.optionNames && item.optionNames.length > 0 ? (
                      <div className="flex flex-wrap gap-1">
                        {item.optionNames.map((name, i) => (
                          <Badge key={i} className="bg-[var(--accent-light)] px-1.5 py-0.5 text-[0.6rem] font-medium text-[var(--brand-orange-dark)]">
                            +{name}
                          </Badge>
                        ))}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {Object.entries(item.ingredientCustomizations)
                          ...
                          .map(([ingId, qty]) => {
                            ...
                            return (
                              <Badge key={ingId} className={`px-1.5 py-0.5 text-[0.6rem] font-medium ${qty === 0 ? "bg-red-50 text-red-500" : "bg-[var(--accent-light)] text-[var(--brand-orange-dark)]"}`}>
                    ...
                    {editingItemId === item.id ? (
                      <div className="mt-1 rounded-xl border border-[var(--line)] bg-[var(--cream)] p-2.5">
                        ...
                        <div className="mt-2 flex gap-1.5">
                          <Button className="px-3 py-1 text-[0.72rem]" onClick={() => saveNotes(item.id)} size="xs" variant="success">
                            Salvar
                          </Button>
                          <Button className="px-3 py-1 text-[0.72rem] text-[var(--muted)] hover:text-[var(--foreground)]" onClick={() => setEditingItemId(null)} size="xs" variant="secondary">
                            Cancelar
                          </Button>
                        </div>
                      </div>
                      ...
                      >
                        <svg className="mt-0.5 h-3 w-3 shrink-0 text-[var(--muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                         <Typography className="text-[0.75rem] italic leading-snug text-[var(--ink-soft)]" variant="caption">{item.notes}</Typography>
                      </button>
                    ) : (
                      <button
                    ...
                    <div className="mt-auto flex items-center justify-between gap-2 pt-1">
                      <Typography className="text-[1rem] text-[var(--accent)]" variant="title-md">
        ...
        {state.items.length > 0 && (
          <div className="border-t border-[var(--line)] bg-white px-5 pb-6 pt-4 shadow-[0_-4px_20px_rgba(0,0,0,0.04)]">
            ...
            <div className="mb-4 flex items-center justify-between">
              <Typography className="uppercase tracking-[0.1em]" tone="muted" variant="caption">Total</Typography>
              <Typography className="text-[1.5rem] leading-none" variant="title-lg">
                {displayTotal}
              </Typography>
            </div>
            ...
            <Button className="mt-3 w-full py-2 text-[0.78rem] text-[var(--muted)] hover:text-red-500" onClick={clearCart} variant="unstyled">
              Limpar carrinho
            </Button>
          </div>
        )}
