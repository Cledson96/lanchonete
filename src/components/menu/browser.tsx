    ...
    <section className="pb-14" id="cardapio">
      ...
      <div className="shell pt-5">
        ...
        <div className="mb-4">
          <div className="flex items-end justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              />
              <div>
                <Typography as="h2" className="text-[1.45rem] leading-tight" variant="title-lg">
                  {activeCategory?.name || "Cardápio"}
                </Typography>
                {activeCategory?.description ? (
                  <Typography className="mt-0.5 max-w-sm leading-snug" tone="muted" variant="caption">
                    {activeCategory.description}
                  </Typography>
                ) : null}
              </div>
            ...
            {/* Contador como badge */}
            <Badge className="shrink-0 border bg-white px-3 py-1 text-[0.72rem]">
        ...
        {!paginatedItems.length ? (
          <EmptyState className="mt-8 rounded-[var(--radius-lg)] bg-white/60 px-6 py-12">
        ...
        {totalPages > 1 ? (
          <div className="mt-10 flex flex-wrap items-center justify-center gap-1.5">
            <Button
              className="border border-[var(--line)] bg-white px-4 py-2 text-[0.82rem] text-[var(--muted)] hover:border-[var(--green-rich)] hover:text-[var(--green-rich)]"
            ...
            {Array.from({ length: totalPages }, (_, index) => index + 1).map((page) => (
              <Button
                ...
                className={`h-9 min-w-9 px-3 text-[0.82rem] ${
                  page === currentPage
